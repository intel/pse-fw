 ...
 * This example demonstrates how to create services in
 * User Mode and access a shared variable between two services.
 * @{
 */
...
__kernel extern struct k_fifo g_serv0_fifo;
...
/** Declare a global buffer/variable which need to share accross multiple
 * Apps.
 */
 ...
 */
DEFINE_USER_MODE_APP(5, USER_MODE_SERVICE | K_PART_SHARED, service_main, 1024,
		     (void **)obj_list, 5, NULL);

/* Sub Tasks which can access resource allocated to this service(1). */
static void service_sub_task1(void *q1, void *q2, void *q3)
{
	...
	printk("Main Service 5 Task 0\n");

	strcpy(fifo_tx_buff.buff, "hello this message from task 0 Service 1\n");
	...
	printk("Main Service 5 Task 0 done\n");
}

static void service_sub_task2(void *q1, void *q2, void *q3)
{
	...
	printk("Main Service 5 Task 1\n");

	fifo_rx_buff = k_fifo_get(&g_serv0_fifo, K_FOREVER);
	printk("%s\n", fifo_rx_buff->buff);

	printk("Main Service 5 Task 1 done\n");
}

/* @brief service_main function
 * Service Main Entry.
 * Note: The service main thread will be invoked in coopertive mode and
 * service main entry will be invoked even before main function.
 * Service main should complete as quickly as possible and offload all its
 * time consuming work to a sub task to improve boot latency
 */
static void service_main(void *p1, void *p2, void *p3)
{
