...
#define MAX_HOST_SIZE \
	(PAGE_BITMAP_NUM * BITS_PER_DW * CONFIG_HECI_PAGE_SIZE)
...
typedef struct {
	...
	uint8_t minor_ver;
	uint8_t major_ver;
} __packed heci_version_resp_t;

typedef struct {
	uint8_t command;
	uint8_t client_req_bits;
	uint8_t reserved[2];
} __packed heci_host_enum_req_t;

typedef struct {
	uint8_t command;
	uint8_t reserved[3];
	uint32_t valid_addresses[8];
} __packed heci_host_enum_resp_t;

typedef struct {
	uint8_t command;
	uint8_t address;
	uint8_t reserved[2];
} __packed heci_client_prop_req_t;
...
} __packed heci_client_prop_resp_t;
...
typedef struct {
	...
	uint8_t dma_enabled       : 1;
	uint8_t reserved[3];
} __packed heci_client_properties_t;
...
} __packed heci_add_client_req_t;

typedef struct {
	uint8_t command;
	uint8_t client_addr;
	uint8_t status;
} __packed heci_add_client_resp_t;
...
typedef struct {
	...
	uint8_t host_addr;
	uint8_t reserved;
	dma_msg_info_t dma_buf[0];
} __packed heci_bus_dma_xfer_resp_t;
...
typedef struct {
	...
	uint32_t msg_length;
	uint8_t reserved2[4];
} __packed heci_bus_dma_xfer_req_t;

typedef struct dma_buf_info {
	uint32_t buf_size;
	/*
...
} __packed dma_buf_info_t;

typedef struct {
	uint8_t command;
	uint8_t reserved[3];
	dma_buf_info_t alloc_dma_buf[0];
} __packed heci_bus_dma_alloc_notif_req_t;

typedef struct {
	uint8_t command;
	uint8_t status;         /* 0 = success */
	uint8_t reserved[2];
} __packed heci_bus_dma_alloc_resp_t;
