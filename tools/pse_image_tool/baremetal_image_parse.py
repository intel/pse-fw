...
CROSS_COMPILE_TARGET='arm-none-eabi'
...
if __name__ == "__main__":
    ...
    print("parsing baremetal image...\n")

    f_map = open("./fragments/zephyr.map"); #WA for image stish script
    for line in f_map:
    ...
    obj_copy = os.environ.get('ARMGCC_DIR') + \
                '/bin/' + \
               CROSS_COMPILE_TARGET + '-objcopy'
    f_elf = "./fragments/app.elf"

    readelf = os.environ.get('ARMGCC_DIR') + \
               '/bin/' + \
               CROSS_COMPILE_TARGET + '-readelf'

    Command = [readelf, '-l', './fragments/app.elf']
